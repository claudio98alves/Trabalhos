...
<c:choose>
    <c:when test="${session.option == 'album'}">
        <s:form action="insert">
            <s:textfield label="Title" name="text"></s:textfield>
            <s:textarea label="Description" name="description" cols="40" rows="10"/>
            <s:submit />
        </s:form>
    </c:when>
    <c:when test="${session.option == 'artist'}">
        <s:form action="insert">
            <s:textfield label="Name" name="text"></s:textfield>
            <s:textarea label="Description" name="description" cols="40" rows="10"/>
            <s:submit />
        </s:form>
    </c:when>
    <c:when test="${session.option == 'music'}">
        <s:form action="insert">
            <s:textfield label="Name" name="text"></s:textfield>
            <s:textfield label="Compositor" name="compositor"/>
            <s:textfield label="Duration" name="duracao"/>
            <s:textfield label="Genre" name="genre"/>
            <s:select list="teste" label="Albuns" name="idalbum">
            </s:select>
            <s:select list="teste2" label="Artists" name="idartist">
            </s:select>
            <s:submit />
        </s:form>
    </c:when>
